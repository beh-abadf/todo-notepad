0_ Run 'composer install' command for missing packages or files.

1_ Run server

2_ Make a mysql database and name it 'todo_notepad_db' and import '.sql' file in dummy_data folder.

3_ For implementing 'fogetting password' system and other services, needs local server configuration in .env file! 

4_ If there is no '.env' file make one with command 'cp .env.example  .env' and generate
application key with command 'php artisan key:generate' and configure '.env' file with your local server configuration.

5_ Run 'npm install' command for node_modules.

6_ Run 'npm run build' command.

Note*: debug mode is off.

//*******************


Written and devoloped by Behrang Abad Foomani
